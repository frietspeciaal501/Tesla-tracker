        ...
        if row is None:
            cur.execute(
                ...
                INSERT INTO listings
                    (id, source, title, price, year, mileage, url, first_seen, last_seen, removed_on)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
                """,
                (
                    item["id"], item["source"], item["title"], item["price"],
                    item["year"], item["mileage"], item["url"], today, today,
                ),
            )
            new_count += 1
        else:
            cur.execute(
                "UPDATE listings SET last_seen = ?, removed_on = NULL, price = ? WHERE id = ?",
                (today, item["price"], item["id"]),
            )
